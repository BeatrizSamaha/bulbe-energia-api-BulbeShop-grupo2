...
export const listarProdutosPorLoja = (req, res) => {
  try {
    const loja = db.prepare('SELECT id FROM lojas WHERE id = ? AND ativa = 1').get(Number(req.params.id));

    if (!loja) {
      return res.status(404).json({ erro: 'Loja parceira não encontrada.' });
    }

    const produtos = db.prepare('SELECT * FROM produtos WHERE loja_id = ?')
      .all(loja.id)
      .map(parseProduto);

    return res.status(200).json(produtos);
  } catch (error) {
    return res.status(500).json({ erro: 'Erro interno ao buscar produtos da loja.' });
  }
};
